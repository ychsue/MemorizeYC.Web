...
class PlayOneCategoryPageController{
    ...
    //* [2018-01-11 11:28] Added for Content Synthesis
    private _currentContentSynVoice: SpeechSynthesisVoice_Instance;
    get currentContentSynVoice(): SpeechSynthesisVoice_Instance { return this._currentContentSynVoice; }
    set currentContentSynVoice(value: SpeechSynthesisVoice_Instance) {
    ...
    constructor($scope, $routeParams) {
        ...
        $(this.dlFinish).dialog({
            ...
            MyFileHelper.FeedTextFromTxtFileToACallBack(
                ...
                private _isWCardUtterPausing;
                public get isWCardUtterPausing() : boolean {
                    return PlayOneCategoryPageController.Current._isWCardUtterPausing;
                }
                public set isWCardUtterPausing(v : boolean) {
                ...
                public onPlayAllViewableWCard(ev) {
                    ...
                    // let isPausing = false; //use to stop the recursive play
                    ...
                    public Larger_Click = function (ev: Event) {
                        ...
                        public GetCurrentSynVoice() {
                            ...
                            PlayOneCategoryPageController.scope.$apply(() => {
                                ...
                                PlayOneCategoryPageController.Current.currentSynVoice = GlobalVariables.currentSynVoice;
                                    ...
                                    //* [2018-01-11 14:23] Gotten ContentCurrentVoice, too
                        ...
                        */
                        public PlayAudio(wCard: WCard, callback:any=null) {
                            if (wCard.cardInfo.AudioFilePathOrUri && (PlayOneCategoryPageController.Current.playType==="hint" && PlayOneCategoryPageController.Current.IsDictateTextContentInHint === true)===false) {
                                ...
                                meAud.src = CardsHelper.GetTreatablePath(wCard.cardInfo.AudioFilePathOrUri,
                                    ...
                                    if (GlobalVariables.synthesis.paused)
                                    GlobalVariables.synthesis.resume();
                                    //* [2018-01-11 14:58] Try to make it can dictact in two kinds of language
                                    ...
                                    if (isHintMode) {
                                        let contentSentence = $(wCard.cCards[wCard.boxIndex]).text();
                                        let contentLang = (PlayOneCategoryPageController.Current.ContentSynLang)? PlayOneCategoryPageController.Current.ContentSynLang: PlayOneCategoryPageController.Current.SynLang;
                                        let contentVoice = (PlayOneCategoryPageController.Current.currentContentSynVoice)? PlayOneCategoryPageController.Current.currentContentSynVoice: PlayOneCategoryPageController.Current.currentSynVoice;
                                        let isAns = PlayOneCategoryPageController.Current.IsDictateAnsInHint;
                                        let isContentSyn = PlayOneCategoryPageController.Current.IsDictateTextContentInHint;
                                        let isAns1st = PlayOneCategoryPageController.Current.isAnsFirst
                                        
                                        // Remember that, langs, voices and sentences are stacks (LIFO) but not queues (FIFO)
                            ...
                            public speakRecursively(sentences:Array<string>, langs:Array<string>, voices:Array<SpeechSynthesisVoice_Instance>, callback:any){
                                if(sentences.length<=0){
                                    ...
                                    {
                                        let s = sentences.slice(0);
                                        let l = langs.slice(0);
                                        let v = voices.slice(0);
                                        if(sentence=="")
                                        {
                                            PlayOneCategoryPageController.Current.speakRecursively(s,l,v,callback);
                                        }
                                        else{
                                            $(GlobalVariables.synUtterance).one("end",()=>{
                        ...
                        function ShowWCardsAndEventsCallback(jsonTxt: string, restWcards: WCard[]) {
                            ...
                            //* [2018-01-11 11:56] Added for ContentSynLang
